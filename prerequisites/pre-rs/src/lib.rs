...
fn transfer_sol() {
    ...
    let to_pubkey = Pubkey::from_str("92uPK3KNcN3bmXP8KzC8KFx12rdRmJfFUVdtc3ei5YvV").unwrap();
...
fn mint_rs() {
    const RPC_URL: &str = "https://api.devnet.solana.com";
    let rpc_client = RpcClient::new(RPC_URL);
    let signer = Keypair::from_bytes(&bs58::decode("gSxVf78iBqb4eNtiMvRFV2xjeUXeGEDXmwHHMABQWd8o4wVYJh3Py3WryLQQXCQoWHKxkWzoaCRjuuVQT9HuAVF").into_vec().unwrap())
        .expect("Failed to create keypair from base58 string");
    ...
    let accounts = vec![
        ...
        // Derive the authority PDA using seeds: b"collection" and collection pubkey
